window.currentPost = {
  ...
  content: `
当サイト(mokuyoubi.org)は当初**Next**製として誕生した。ところが、小さなサイトのためにはあまりにも大袈裟なフレームワークだという感じがした。
そこで移行先を検討したところ、コードがとても直感的に思えた**Svelte**に白羽の矢が立った。そうして、移行した。
...
ではそのコードとは何か？普段Expo開発をしている自分にとっては、Nextでないならば**Vanilla(素のJS)**くらいしかないわけだが、それはMarkdownが使えない、つまりHtmlでブログを書くことだと僕は思い込んでいた。
そんな中、ふと、ブログは**JSの文字列でも書ける**のでは、と気づいたのだ。
考えてみれば、**Markdownが使えるフレームワークも内部で行なっていることは結局はMarkdownのHtmlへの変換**だ。なら、**それをJSで自分でやればいい**。

これはめちゃめちゃワクワクする発想だった！
というのは、**独自の記法を作ることができる**からだ。
Markdownに対して、なんで改行で改行しないわけ？などの疑問があったとしても、**そんなことを気にしなくていい、自分にとって使いやすい記法**を作れるのだ。

Vanillaへの引っ越しに伴い、**Tailwind**も導入した。これは、Expoにも**Nativewind**というものがあるので、もし使いやすければExpoに導入するのもいいかもなと思ったのだ。
